...
class TrajNode(Node):
    def __init__(self):
        ...
        self.recorded_data = [] # List of (t, pos, vel)
        self.rec_first_msg = True
        self.rec_t0 = 0.0
    ...
    def on_state(self, msg: JointState):
        ...
        with self.data_lock:
            ...
            if self.recording:
                if self.rec_first_msg:
                    self.rec_t0 = t_sec
                    self.rec_first_msg = False
                
                # Store relative time (ROS time), pos, vel
                t_rel = t_sec - self.rec_t0
                self.recorded_data.append((t_rel, self.pos.copy(), self.vel.copy()))
...
class TrajGUI:
    def __init__(self, node: TrajNode):
        ...
        self.root = tk.Tk(); self.root.title(f"V{self.version} - CAN Trajectory") # Versión en el título
        ...
        self.auto_pause = tk.BooleanVar(value=True) # Default: True for better performance # Default to False for live plotting
        ...
        self._build_ui()
        self._build_plot()
        self._update_visibility(rebuild=True)
    ...
    def _build_ui(self):
        ...
        f3 = ttk.Frame(self.root, padding=(6,2)); f3.pack(side=tk.TOP, fill=tk.X)
        ...
        grid = ttk.Frame(self.root, padding=(6,2)); grid.pack(side=tk.TOP, fill=tk.X)
        ...
        ttk.Label(self.root, text="* Si eliges DEG, introduce objetivos en grados; internamente se convierten a rad. J5 siempre en mm.", foreground="#555").pack(side=tk.TOP, anchor='w', padx=8, pady=(2,6))
    ...
    def _build_plot(self):
        self.fig, (self.ax_q, self.ax_dq, self.ax_ddq) = plt.subplots(3, 1, figsize=(9.0, 7.5), sharex=True, constrained_layout=True)
        
        self.ax_q.set_title("Posición y Error"); self.ax_dq.set_title("Velocidad y Error"); self.ax_ddq.set_title("Aceleración y Error")
        self.ax_ddq.set_xlabel("Tiempo [s]")
        
        self.lines_q, self.lines_dq, self.lines_ddq = [], [], []
        ...
        for j in range(N_JOINTS):
            ...
            # Posición (Setpoint, Medida, Error)
            lq_sp, = self.ax_q.plot([], [], label=f"{name} SP", color=c, linestyle='-', picker=5)
            lq_pos, = self.ax_q.plot([], [], label=f"{name} Medido", color=c, linestyle='--', picker=5)
            # ERROR CON COLOR DEL JOINT (Punteado)
            lq_err, = self.ax_q.plot([], [], label=f"{name} Error", color=c, linestyle=':', linewidth=2, alpha=0.8, picker=5)
            self.lines_q.extend([lq_sp, lq_pos, lq_err])

            # Velocidad (Setpoint, Medida, Error)
            ldq_sp, = self.ax_dq.plot([], [], label=f"{name} SP", color=c, linestyle='-', picker=5)
            ldq_vel, = self.ax_dq.plot([], [], label=f"{name} Medido", color=c, linestyle='--', picker=5)
            ldq_err, = self.ax_dq.plot([], [], label=f"{name} Error", color=c, linestyle=':', linewidth=2, alpha=0.8, picker=5)
            self.lines_dq.extend([ldq_sp, ldq_vel, ldq_err])

            # Aceleración (Setpoint, Medida, Error)
            # Aceleración (Setpoint, Medido, Error)
            lddq_sp, = self.ax_ddq.plot([], [], label=f"{name} SP", color=c, linestyle='-', picker=5)
            lddq_acc, = self.ax_ddq.plot([], [], label=f"{name} Medido", color=c, linestyle='--', picker=5)
            lddq_err, = self.ax_ddq.plot([], [], label=f"{name} Error", color=c, linestyle=':', linewidth=2, alpha=0.8, picker=5)
            self.lines_ddq.extend([lddq_sp, lddq_acc, lddq_err])

        self._set_ylabels() # Ensure labels are set initially

        frame = ttk.Frame(self.root); frame.pack(side=tk.TOP, fill=tk.BOTH, expand=True)
        ...
        # Hover annotations (one per axes)
        self.annots = {}
    ...
    def _savgol_filter(self, y, window_length, polyorder, deriv=0, delta=1.0):
        """
        Apply a Savitzky-Golay filter to an array using NumPy.
        """
        try:
            ...
            # Compute coefficients (only once ideally, but this is fast enough)
            x = np.arange(-half_window, half_window + 1)
            # Design matrix A: A[i, j] = x[i] ** j
            # We use x^0, x^1, ... x^polyorder
            A = np.vander(x, polyorder + 1)[:, ::-1]
            
            # Pseudo-inverse: (A.T @ A)^-1 @ A.T
            m = np.linalg.pinv(A)
            
            # The row corresponding to the requested derivative (0 for smooth, 1 for 1st deriv, etc.)
            coeffs = m[deriv]
            
            # Apply convolution
            # mode='same' returns output of length max(M, N). Boundary effects are handled by zero-padding in convolve,
            # which isn't ideal for polynomial fitting at edges, but acceptable for this use case if we ignore edges.
            # Better: use 'valid' and pad, or just accept edge artifacts.
    ...
    def _update_plot(self, units_changed=False):
        ...
        if self._running:
            ...
            target_points = 2000 # High detail when stopped
        ...
        # Apply Smoothing for Plotting (Visual only) - REMOVED
        # We now process data in _run_generator for better quality
        # for j in range(N_JOINTS):
        #     dq_vel_arr[:, j] = self._smooth(dq_vel_arr[:, j], alpha=0.3)
        #     ddq_accel_arr[:, j] = self._smooth(ddq_accel_arr[:, j], alpha=0.1)
    ...
    def _update_visibility(self, rebuild=False):
        for j in range(N_JOINTS):
            vis = self._visible_for_joint(j)
            for lines in [self.lines_q, self.lines_dq, self.lines_ddq]:
                for k in range(3):
                    lines[j * 3 + k].set_visible(vis)
        
        if rebuild: self._rebuild_legends()
    ...
    def _threaded_data_recorder(self):
        ...
        while self._recording:
            try:
                # FIX: Pause recorder during movement (handled by generator)
                if self._running:
                    time.sleep(0.1)
                    continue
    ...
    def _plot_updater(self):
        """Low frequency loop for updating the plot (20Hz)"""
        try:
            # Optimization: COMPLETELY SKIP if running to save CPU
            if self._running:
                self.plot_after_id = self.root.after(500, self._plot_updater)
                return

            if not self._paused:
                self._update_plot()
            
            # Reduce update rate to 10Hz (100ms) to save CPU
            self.plot_after_id = self.root.after(100, self._plot_updater) 
        except Exception as e:
            print(f"Error in plot updater: {e}")
            self.plot_after_id = self.root.after(100, self._plot_updater)
    ...
    def _run_generator(self, mode, cur, tgt, Tvec, dt, sel_values):
        ...
        try:
            ...
            sp_history = [] # (t, q_sp, dq_sp, ddq_sp)
            ...
            while self._running:
                ...
                # 1. Polling (approx every 50ms)
                steps_per_poll = int(0.05 / dt)
                if steps_per_poll < 1: steps_per_poll = 1
                
                if idx % steps_per_poll == 0:
                    # Targeted Polling
                    for j in range(4):
                        if sel_values[j]:
                            self.node.pub_cmd.publish(String(data=f"A{j+1}"))
                    if sel_values[4]:
                        self.node.pub_cmd.publish(String(data="A5"))

                # Store Setpoint for later merging
                sp_history.append((elapsed, p, v, self.traj_plan[2][idx]))
            ...
            raw_data = self.node.stop_recording()
            
            if len(raw_data) > 0 and len(sp_history) > 0:
                # Process and Merge
                # 1. Prepare Arrays
                t_sp = np.array([x[0] for x in sp_history])
                q_sp = np.array([x[1] for x in sp_history])
                dq_sp = np.array([x[2] for x in sp_history])
                ddq_sp = np.array([x[3] for x in sp_history])
                
                t_meas = np.array([x[0] for x in raw_data])
                q_meas_raw = np.array([x[1] for x in raw_data])
                dq_meas_raw = np.array([x[2] for x in raw_data])
                
                # 2. Interpolate Measured Data onto Setpoint Timebase
                q_meas = np.zeros_like(q_sp)
                
                # Calculate average dt for filter scaling
                dt_avg = np.mean(np.diff(t_sp)) if len(t_sp) > 1 else 0.01
                
                for j in range(N_JOINTS):
                    q_meas[:, j] = np.interp(t_sp, t_meas, q_meas_raw[:, j])
                    # SG FILTER: Smooth Position (Window 31, Order 2)
                    q_meas[:, j] = self._savgol_filter(q_meas[:, j], window_length=31, polyorder=2, deriv=0)
                    
                # 3. Calculate Velocity from Position (SG Filter Derivative)
                dq_meas = np.zeros_like(q_meas)
                for j in range(N_JOINTS):
                    # SG Filter 1st Derivative (Window 31, Order 2)
                    dq_meas[:, j] = self._savgol_filter(q_meas[:, j], window_length=31, polyorder=2, deriv=1, delta=dt_avg)

                # 4. Calculate Acceleration (SG Filter Derivative of Velocity)
                ddq_meas = np.zeros_like(dq_meas)
                for j in range(N_JOINTS):
                    # SG Filter 1st Derivative of Velocity (Window 31, Order 2)
                    ddq_meas[:, j] = self._savgol_filter(dq_meas[:, j], window_length=31, polyorder=2, deriv=1, delta=dt_avg)
                    
                # 5. Update History Buffer
                self._reset_history()
                # Push all points
                for i in range(len(t_sp)):
                    self._push_history(t_sp[i], q_sp[i], dq_sp[i], ddq_sp[i], 
                                     q_meas[i], dq_meas[i], ddq_meas[i])
                                     
                print(f"Data Processing Complete: {len(t_sp)} points merged.")
                ...
                try:
                    ...
                    debug_file = "debug_traj_last_run.csv"
                    header = "t_sp,q_sp,dq_sp,q_meas,dq_meas,ddq_meas"
                    # Just save J4 (index 3) for quick check, or all? Let's save J4
                    # Find active joint
                    j_active = 3 # Default J4
                    for j in range(5):
                        if sel_values[j]: j_active = j; break
                        
                    data_debug = np.column_stack((
                        t_sp, 
                        q_sp[:, j_active], 
                        dq_sp[:, j_active],
                        q_meas[:, j_active],
                    ...
                    np.savetxt(debug_file, data_debug, delimiter=",", header=header, comments="")
                    print(f"DEBUG: Saved raw data to {debug_file}")
                except Exception as e:
    ...
    def _poll_A_loop(self):
        ...
        while self._polling:
            try:
                # FIX: Pause polling during movement (handled by generator)
                if self._running:
                    time.sleep(0.1)
                    continue
                else:
